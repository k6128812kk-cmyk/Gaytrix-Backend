...
import { notifyAdmins } from '../bot/bot';
...
// Admin notifications are sent on every new verification request.
// ==========================================================================
...
export async function requestVerification(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const userInfo = await db.query(
      `SELECT telegram_id, telegram_username, display_name FROM users WHERE id = $1`,
      [req.user!.id]
    );
    if (userInfo.rows[0]) {
      const u = userInfo.rows[0];
      notifyAdmins('verification_request', {
        userId: req.user!.id,
        telegramId: u.telegram_id,
        telegramUsername: u.telegram_username,
        details: `Display name: ${u.display_name || 'N/A'}`,
      }).catch(() => {});
    }
  ...
  try {
    ...
    try {
      ...
      if (userInfo.rows[0]) {
        await sendNotification(
          userInfo.rows[0].telegram_id,
          n(userInfo.rows[0].language_preference, 'verificationRejected')
        );
      }
    } catch { /* notification failure is non-fatal */ }
